...
public Action ChatListener(int client, const char[] command, int args)
{
	...
	else if (g_bWaitingForWear[client] && IsValidClient(client) && g_iGloves[client][playerTeam] != 0 && !IsChatTrigger()) {
	...
	if (StrEqual(msg, ".gloves") || StrEqual(msg, ".glove"))
	{
		CommandGlove(client, 0);
	}
